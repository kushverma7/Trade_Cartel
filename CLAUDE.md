...
2. `MEMORY.md` — persistent context, decisions, standing rules. **Its final section, "CURRENT STATE AND NEXT ACTION", is the session handoff — read it first; it replaces having the previous conversation.**
...
LEARNING CAPTURE (mandatory): every session that produces analysis, backtest results, or user trade feedback must update BELIEF_REGISTER.md / PLAYBOOK.md / MEMORY.md before ending. **ARCHIVE THE TRANSCRIPT TOO** (2026-08-17): the session JSONL at `/root/.claude/projects/.../<session-id>.jsonl` lives on EPHEMERAL container disk and is lost when the container recycles — as the uploads directory was, mid-session, on 2026-08-16. Copy it gzipped into `trader_playbooks/sources/session_archive/` and extract the user turns verbatim alongside it, EARLY and again at the end, not only at the end. **RUN EVERY ARCHIVE THROUGH `session_archive/redact.py` FIRST** (2026-08-17): this repo is PUBLIC and a live API key was pasted into a session on that date. Anything the user pastes that looks like a credential is redacted before it is written, and `python3 session_archive/redact.py` audits the whole archive directory. Never commit a secret; tell the user to rotate it instead. Uploaded files are equally ephemeral: anything the user sends that matters must be committed to the repo the same session it arrives. Every code bug found must be added to bugs/BUG_REGISTRY.md; every validated pattern to skills/SKILL_REGISTRY.md. If it isn't captured, it wasn't learned. **Do the capture when the finding lands, not at session end — sessions get cut off.**
