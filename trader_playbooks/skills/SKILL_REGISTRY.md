...
| **Offline backtest engine** | backtest/engine.py + trend.py | Python research pipeline: non-lookahead levels, trailing-stop trend engine, train/test split, matched-n random null, deflated Sharpe | **CROSS-VALIDATED against TradingView Pine 2026-07-31** — identical trade count (38) and win rate (26.32%) on the same window/settings, PF within 0.017. Plus 5 hand-computed accounting tests | gold_trend_trailing.pine research | BUG-012 |
| **Pine v6 static linter** | backtest/pine_lint.py | Continuation-indent, block-body, duplicate-declaration, delimiter-balance and BUG-014 (exit without stop) checks | Written after two ad-hoc versions produced false alarms; both linter bugs found and fixed by running it against known-good files | all .pine deliveries | BUG-014 |
