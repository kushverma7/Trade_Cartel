# OMNIBUS PROTOCOL — ADOPTED, WITH CORRECTIONS
Adopted 2026-07-22 as the standing iteration/operating loop, per user
directive. Original archived verbatim at
`trader_playbooks/sources/amdm_synthesis/OMNIBUS_PROTOCOL_original.md`.
The full original text follows below the correction note.

## ADOPTION NOTE — corrections applied per this repo's honesty rules

1. **The "19 Voices" map contains one fabricated voice and several
   register mismatches — the REPO REGISTER WINS wherever they differ:**
   - **"#19 Jared Tendler (HIGH, psychology)" does not exist in this
     project.** No Tendler material was ever supplied, no playbook
     exists, and the register's actual voice #19 is **Hima Reddy**
     (Trading Methodologies of W.D. Gann, MEDIUM credibility). Do NOT
     cite Tendler concepts as if they were ingested source material.
     If the user supplies real Tendler material later, it becomes a
     new voice through the normal extraction pipeline.
   - The register holds **21 voices, not 19** (the map also omits the
     Candlestick/Chart-Pattern foundational layer and conflates the
     Valentini/orderflow pair).
   - Credibility grades in the map are looser than the audited
     register in several spots (e.g. map says Trader Dale HIGH,
     register says MEDIUM; map says Alchemist Med-High, register says
     LOW-MED; map says Ario Medium, register says LOW). The audited
     per-voice headers in `trader_playbooks/*.md` are authoritative —
     the map below is a quick-reference sketch only.
2. **"Discarded" in the map means discarded FROM THE AMDM SYNTHESIS,
   not from the repo.** Daye (#4), Yotov (#18), Gann (#16), and
   Kurisko (#3) all remain fully built, playbooked, and maintained per
   the never-discard rule — they are simply not components of the
   current AMDM model.
3. **The iteration loop's "deploy & observe" phase requires the USER's
   chart/backtest results** — this environment cannot run TradingView.
   The loop therefore cycles as: build -> static-validate -> deliver ->
   USER tests -> feed results back -> diagnose (3 minds) -> fix ->
   repeat. Iterations are driven by each results report; between
   reports, the static phases (truth table, bug hunt, desk check —
   already mandated by CODE_DELIVERY_PROTOCOL.md) still run in full.
4. **The improvement-target tables (20/40/60/80 trades/month etc.) are
   goalposts, not claims.** Nothing gets labeled Minimum Viable/Good/
   Excellent without a real sample per the PF 3.656 retraction rule.

The Bug Hunt Checklist below overlaps CODE_DELIVERY_PROTOCOL.md's
phases; both run — the checklist is the per-iteration quick pass, the
7-phase protocol is the full pre-delivery gate. Bugs found land in
`bugs/BUG_REGISTRY.md` using its format (which already includes the
"which mind found it" field's equivalent).
