...
| `msvp_v3_four_trader_hybrid.md` / `MSVP_v3_Four_Trader_Hybrid.pdf` | Four-trader hybrid v3.0. **Verification note: Section 5 claims "Full Code" but contains only a placeholder comment — there is no script.** Its central rule ("more conditions = higher probability", minimum 4-5 confluences) is contradicted by three earlier documents in the same series that measured it producing near-zero signals. Section 6's backtest plan is sound. |
